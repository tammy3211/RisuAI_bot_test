...
export class RegexModule {
    ...
    private renderList(): void {
        ...
        this.regexData.forEach((entry, index) => {
            ...
            entryDiv.innerHTML = `
                ...
                <div class="toolbar">
                    <button class="btn-small" data-index="${index}" data-action="test">테스트</button>
                    ${entry.mdFile ? `<button class="btn-small btn-secondary" data-index="${index}" data-action="edit">MD 편집</button>` : ''}
                </div>
            `;
            
            // Add event listeners
            const testBtn = entryDiv.querySelector('[data-action="test"]');
            const editBtn = entryDiv.querySelector('[data-action="edit"]');
            
            testBtn?.addEventListener('click', () => this.testEntry(index));
    ...
    private testRegex(): void {
        ...
        const testText = (document.getElementById('regex-test-text') as HTMLTextAreaElement)?.value || '';
        const replacement = (document.getElementById('regex-replacement') as HTMLInputElement)?.value || '';
    ...
    testEntry(index: number): void {
        ...
        if (patternInput) patternInput.value = entry.in;
        if (replacementInput) replacementInput.value = entry.mdContent || entry.out;
